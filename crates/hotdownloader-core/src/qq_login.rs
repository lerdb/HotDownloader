    ...
    async fn next_event(&mut self, timeout: Duration) -> Result<Option<MqttLoginEvent>, String> {
        ...
        loop {
            ...
            match packet {
                Some(Packet::Publish(publish)) => {
                    if let Some(event) = parse_publish_event(&publish) {
                        // PUBLISH 中可能包含音乐 Key，日志只记录事件到达。
                        log::info!("[登录] 收到 PUBLISH 事件");
...
pub async fn create_qr_login(store: Arc<dyn LoginCredentialStore>) -> Result<String, String> {
        ...
        loop {
            ...
            loop {
                match mqtt.next_event(MQTT_EVENT_WAIT_TIMEOUT).await {
                    Ok(Some(event)) => {
                        // 事件可能携带登录令牌，不在日志中打印整个载荷。
                        log::info!("[登录] 收到 MQTT 事件");
                        let should_break = match &event {
                            ...
                            MqttLoginEvent::Cookies {
                                ...
                                log::info!("[登录] 收到 Cookies 事件，music_id = {}", music_id);
                                // 调用 Login 接口换取正式凭据。
                                ...
                                .await
                                {
                                    Ok(login_data) => {
                                        ...
                                        match creds_result {
                                            Ok(creds) => {
                                                if let Err(e) =
                                                    save_credentials(store.as_ref(), &creds).await
                                                {
...
pub async fn check_qr_login(qrcode_id: String) -> Result<String, String> {
...
mod tests {
    ...
    async fn file_store_keeps_non_login_settings_when_logging_out() {
        ...
        let status = get_login_status(&store).await.unwrap();
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(&status).unwrap()["logged_in"],
            true
        );
