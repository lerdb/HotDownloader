...
impl TaskController {
    /// 暂停期间等待恢复或取消；先检查标志，再订阅通知，避免恢复与等待交错时卡住。
    /// 返回 true 表示任务被取消，调用方应直接退出下载循环。
    pub async fn wait_for_resume_or_cancel(&self) -> bool {
        loop {
            ...
            tokio::select! {
                _ = self.cancel_token.cancelled() => return true,
...
pub struct DownloadEngine {
    runner: Arc<dyn DownloadTaskRunner>,
        ...
        loop {
            ...
            while let Some(ctx) = {
                ...
                if let Some(stale_id) = stale_task_id {
                    ...
                    self.runner
                        .report_error(&stale_id, "任务已失效，请重新添加或重试");
                    continue;
                }
                ...
                if let Some(ctrl) = ctrl {
                    ...
                    tokio::spawn(async move {
                        ...
                        let completed_ok =
                            match std::panic::AssertUnwindSafe(runner.run(ctx, ctrl_clone.clone()))
                                .catch_unwind()
                                .await
                            {
                                Ok(ok) => ok,
                        ...
                        if completed_ok {
                            // 平台适配器决定是否发送系统通知，核心调度器不读取 Tauri 设置。
                            notifier.completed(&notify_song_title, &notify_artist);
    ...
    pub async fn remove(&self, task_id: &str, delete_file: bool) -> Result<(), String> {
        ...
        if delete_file {
            if let Some(path) = &final_path {
                if let Err(e) = self.file_deleter.remove(path).await {
        ...
        if delete_file {
            if let Some(lrc) = self.lrc_paths.lock().await.remove(task_id) {
                if let Err(e) = self.file_deleter.remove(&lrc).await {
...
mod tests {
    ...
    impl DownloadTaskRunner for StubRunner {
        fn run(
            &self,
        ...
        fn report_error(&self, _task_id: &str, message: &str) {
            panic!("意外的调度错误: {message}");
        }
    }
    ...
    #[tokio::test]
    async fn paused_controller_exits_when_canceled() {
        let controller = TaskController {
            cancel_token: CancellationToken::new(),
        ...
        assert!(controller.wait_for_resume_or_cancel().await);
    }

    #[tokio::test]
    async fn scheduler_runs_a_task_without_tauri() {
        let runs = Arc::new(AtomicUsize::new(0));
        let hits = Arc::new(AtomicUsize::new(0));
        let done = Arc::new(Notify::new());
        let engine = DownloadEngine::new(
            Arc::new(StubRunner(runs.clone())),
