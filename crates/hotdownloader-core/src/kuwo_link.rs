...
#[cfg(test)]
mod tests {
    use super::parse_quality_filename;

    #[test]
    fn bitrate_and_format_follow_task_filename_contract() {
        assert_eq!(
            parse_quality_filename("320.mp3").unwrap(),
            (320, "mp3".into())
        );
        assert_eq!(
            parse_quality_filename("20900.mflac").unwrap(),
            (20900, "mflac".into())
        );
        assert!(parse_quality_filename("invalid.mflac").is_err());
    }
}
